...
func GeneratePolicyResourceNames(
	...
	case SecretFormatRamen:
		pName = ramenFormatPrefix + secret
	case SecretFormatVelero:
		pName = veleroFormatPrefix + secret
	}

	return pName,
		fmt.Sprintf(secretResourceNameFormat, secretPlBindingBaseName, pName),
		fmt.Sprintf(secretResourceNameFormat, secretPlRuleBaseName, pName),
		fmt.Sprintf(secretResourceNameFormat, secretConfigPolicyBaseName, pName)
}
...
func generatePolicyPlacementName(secret string, format TargetSecretFormat) string {
	...
	return fmt.Sprintf(secretResourceNameFormat, secretPlRuleBaseName, pName)
}
