...
state, application specs) for cross-cluster recovery. S3 profiles are configured in the Ramen hub
operator ConfigMap.
...
`ramen-hub-operator-config` ConfigMap in the `ramen-system` namespace, and are automatically distributed to the managed clusters that form a DRPolicy.
...
- **Same `storageID`** on StorageClasses across clusters → **Metro DR** (sync replication), or
...
- For Regional DR, use different `storageID` values: `storagecluster-instance-east` and `storagecluster-instance-west`
