...
func deleteSecretFromCluster(
	...
	if err := secretsUtil.RemoveSecretFromCluster(
		...
		return fmt.Errorf("drcluster '%v' s3Profile '%v' secrets delete for format '%s': %w",
			clusterName, s3SecretToDelete, util.SecretFormatRamen, err)
	}
	...
	if !ramenConfig.KubeObjectProtection.Disabled && ramenConfig.KubeObjectProtection.VeleroNamespaceName != "" {
		if err := secretsUtil.RemoveSecretFromCluster(
			...
			return fmt.Errorf("drcluster '%v' s3Profile '%v' secrets delete for format '%s': %w",
				clusterName, s3SecretToDelete, util.SecretFormatVelero, err)
		}
	}
